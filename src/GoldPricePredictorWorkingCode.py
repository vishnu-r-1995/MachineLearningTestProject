...
ticker = 'GC=F'
time_period = '20y'
gold_inflation_crude_dji_wmt = get_csv_dataframe('C:\All\Dataset\gold_inflation_crude_dji_wmt_int.csv');
gold_inflation_crude_dji_wmt = gold_inflation_crude_dji_wmt.reset_index()
dataframe_columns_used = ['Open', 'High', 'Low', 'Inflation', 'DJI Adj Close', 
                         'WMT Adj Close', 'Crude Oil Adj Close', 'Interest Rate', 'Close']
...
correlation_matrix = get_correlation_matrix_for_dataframe(gold_inflation_crude_dji_wmt[dataframe_columns_used])

display_scatter_matrix_for_dataframe(gold_inflation_crude_dji_wmt, 
                                     ['Close', 'Crude Oil Adj Close', 'Inflation', 'DJI Adj Close', 
                                      'WMT Adj Close', 'Interest Rate'])
...
train_set, test_set = train_test_split(gold_inflation_crude_dji_wmt, test_size = 0.2, shuffle = False)
...
def Lstm_model():
    ...
    regressor.add(LSTM(units = 50, return_sequences = True, input_shape = (30, number_of_features)))
...
def prep_data_rf(dataframe, lookback, future, scaler, imputer, target_scaler):
    ...
    dataframe_rf_x = dataframe_rf_scaled[:, 0 : number_of_features]
    dataframe_rf_y = dataframe_rf_scaled[:, number_of_features :]

    dates_train_rf = pd.to_datetime(dataframe['Date'])
    #np.array(x) gives a 3D array & np.array(y) gives 2D array
...
def prep_test_data_rf(dataframe, lookback, future, scaler, imputer):
    ...
    dataframe_rf_x = dataframe_rf_scaled[:, 0 : number_of_features]
    dataframe_rf_y = dataframe[['Close']].values

    dates_rf = pd.to_datetime(dataframe['Date']).values
    #np.array(x) gives a 3D array & np.array(y) gives 2D array
...
rmse_rf = mean_squared_error(pd.DataFrame({'RF_Predicted' : predicted_rf_descaled[:,0]}).RF_Predicted,
                             pd.DataFrame({'RF_Real' : rf_test_y[:,0]}).RF_Real, 
                             squared = False)
print('rmse rf', rmse_rf)
write_to_file('RMSE using Random Forest Regression' + str(rmse_rf) + '\n', data_file_path)

mape_rf = mean_absolute_percentage_error(pd.DataFrame({'Real' : rf_test_y[:,0]}).Real, 
...
avg_rmse = mean_squared_error(pd.DataFrame({'Real' : rf_test_y[30:,0]}).Real, ensemble_dataframe['Avg Prediction'], squared=False)
